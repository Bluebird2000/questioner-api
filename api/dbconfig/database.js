...
const { DATABASE_TEST_URL, DATABASE_URL, NODE_ENV } = process.env;
...
if (NODE_ENV === 'development') {
  ...
  const createTestMeetup2 = `INSERT INTO meetups(
          ...
          VALUES(
          'Epic Tower', 
          'Advancing Human potentials By investing in tomorrows leader', 
          '2019-01-03', NOW(), 
          NOW()
          )`;
  ...
  client.connect((err) => {
    if (err) {
      ...
      console.log('connection to test server successful');
    }
  });
  ...
  const tableQuery = `${userTableQuery} ; ${createTestUser} ; ${meetupTableQuery} ${createTestMeetup} ; ${createTestMeetup2}`;
  client.query(tableQuery, (error) => {
    client.end();
    if (error) {
      console.log(error.message);
